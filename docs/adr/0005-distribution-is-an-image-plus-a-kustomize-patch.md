...
`http://127.0.0.1:<port>`, and sets `KUBECONFIG` on the Client's container.
...
(which forfeits the loopback assumption that lets us skip TLS — see ADR 0002) or forking the CSV
into a custom catalog. Both are worse than saying no.
